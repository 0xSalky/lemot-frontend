...
function normalizeSetupContext(raw: unknown): TradeSetupContext {
  ...
  return {
    ...
    markov_posterior_pct: num(row.markov_posterior_pct),
    markov_log_odds_total: num(row.markov_log_odds_total),
    setup_grade: str(row.setup_grade),
    enter_probability_pct: num(row.enter_probability_pct),
    tp_preset: str(row.tp_preset),
    ...
    fractal_volume_tier: str(row.fractal_volume_tier),
    band_type_tier: str(row.band_type_tier),
    band_tf_count: num(row.band_tf_count),
    band_density: str(row.band_density),
    band_has_anchor: row.band_has_anchor == null ? null : Boolean(row.band_has_anchor),
